...
func (h *Handler) Pull(ctx context.Context) error {
	...
	// for each chain update the chain and contracts info
	// If we wanted to be more creative we could first check
	// to see if the file had actually changed since the last time
	// it was pulled
	for _, chain := range h.chains {
		if err := h.getChainContracts(chain); err != nil {
			return err
		}
	}
	...
	// update timestamp
	h.lastUpdated = time.Now()
	h.log.Printf("successfully updated contracts registry (%d chains)", len(h.chains))
...
func (h *Handler) getChainContracts(name string) error {
	query := fmt.Sprintf("https://raw.githubusercontent.com/%s/main/%s/contracts.json", h.registryUrl, name)
	resp, err := http.Get(query)
	if err != nil {
		return err
	}
