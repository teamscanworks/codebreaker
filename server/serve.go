...
// Serve starts a server listening on "listenAddr". In parallel, a cron-like job
...
func Serve(ctx context.Context, registryUrl, listenAddr, updateFreq string) error {
	...
	//v1Router.HandleFunc("/chain/{chain}/address/{address}", handler.Endpoints).Methods("GET")
	v1Router.HandleFunc("/chain/{chain}/code-id/{codeId}", handler.ContractByCodeId).Methods("GET")
